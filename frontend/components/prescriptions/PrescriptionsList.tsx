...
import { safeToLowerCase, safeStringCompare, safeDisplay } from '@/utils/safeUtils';
...
type SortOption = 'diagnosis' | 'patientName' | 'doctorName' | 'prescriptionDate' | 'isActive';
...
export default function PrescriptionsList() {
    ...
    const [sortBy, setSortBy] = useState<SortOption>('diagnosis');
    const [filterBy, setFilterBy] = useState<FilterOption>('all');
    const [favorites, setFavorites] = useState<Set<number>>(new Set());
    const [anchorEl, setAnchorEl] = useState<null | HTMLElement>(null);

    // Buscar prescrições na inicialização
    useEffect(() => {
        fetchPrescriptions();
    }, []);
    ...
    /**
     * Filtra e ordena prescrições baseado nos critérios selecionados
     */
    const filteredAndSortedPrescriptions = prescriptions
        .filter(prescription => {
            const matchesSearch = safeToLowerCase(prescription.diagnosis).includes(searchTerm.toLowerCase()) ||
                safeToLowerCase(prescription.patientName).includes(searchTerm.toLowerCase()) ||
                safeToLowerCase(prescription.doctorName).includes(searchTerm.toLowerCase());

            const matchesFilter = filterBy === 'all' ||
                (filterBy === 'active' && prescription.isActive) ||
                (filterBy === 'inactive' && !prescription.isActive) ||
                (filterBy === 'expired' && prescription.validUntil && isExpired(prescription.validUntil)) ||
                (filterBy === 'valid' && prescription.validUntil && !isExpired(prescription.validUntil));
        ...
        .sort((a, b) => {
            switch (sortBy) {
                case 'diagnosis':
                    return safeStringCompare(a.diagnosis, b.diagnosis);
                case 'patientName':
                    return safeStringCompare(a.patientName, b.patientName);
                case 'doctorName':
                    return safeStringCompare(a.doctorName, b.doctorName);
    ...
    const stats = {
        ...
        expired: prescriptions.filter(p => p.validUntil && isExpired(p.validUntil)).length,
        valid: prescriptions.filter(p => p.validUntil && !isExpired(p.validUntil)).length
    };
     ...
     */
    const handleToggleFavorite = (prescriptionId: number) => {
        setFavorites(prev => {
            const newFavorites = new Set(prev);
            if (newFavorites.has(prescriptionId)) {
                newFavorites.delete(prescriptionId);
            } else {
                newFavorites.add(prescriptionId);
            }
            return newFavorites;
        });
    };
            ...
            {filteredAndSortedPrescriptions.length > 0 && (
                <Grid container spacing={3}>
                    {filteredAndSortedPrescriptions.map((prescription, index) => (
                        <Grid item xs={12} md={viewMode === 'grid' ? 6 : 12} lg={viewMode === 'grid' ? 4 : 12} key={prescription.id}>
                                    ...
                                    <CardContent sx={{ p: 3 }}>
                                        <Box display="flex" justifyContent="space-between" alignItems="flex-start" mb={2}>
                                            <Box display="flex" alignItems="center" flex={1}>
                                                <Avatar
                                                    sx={{
                                                        bgcolor: prescription.isActive ? 'rgba(16, 185, 129, 0.2)' : 'rgba(239, 68, 68, 0.2)',
                                                        color: prescription.isActive ? '#10B981' : '#EF4444',
                                                        mr: 2,
                                                        width: 48,
                                                        height: 48
                                                    }}
                                                ...
                                                <Box flex={1}>
                                                    ...
                                                    >
                                                        {safeDisplay(prescription.diagnosis, 'Diagnóstico não informado')}
                                                    </Typography>
                                                    <Typography
                                                        variant="body2"
                                                        sx={{ color: '#94A3B8' }}
                                                    >
                                                        {safeDisplay(prescription.patientName, 'Paciente não informado')}
                                                    </Typography>
                                                </Box>
                                            </Box>
                                            <Box display="flex" gap={1}>
                                                <Tooltip title={favorites.has(prescription.id) ? "Remover dos favoritos" : "Adicionar aos favoritos"}>
                                                    <IconButton
                                                        size="small"
                                                        onClick={() => handleToggleFavorite(prescription.id)}
                                                        sx={{
                                                            color: favorites.has(prescription.id) ? '#F59E0B' : '#64748B',
                                                            '&:hover': {
                                                                color: favorites.has(prescription.id) ? '#D97706' : '#3B82F6'
                                                            }
                                                        }}
                                                    >
                                                        {favorites.has(prescription.id) ? <Favorite /> : <FavoriteBorder />}
                                                    </IconButton>
                                                </Tooltip>
                                                <Chip
                                        ...
                                        <Box mb={2}>
                                            ...
                                            >
                                                ...
                                                <strong>Médico:</strong> {safeDisplay(prescription.doctorName, 'Não informado')}
                                            </Typography>
                                            <Typography
                                                variant="body2"
                                                display="flex"
                                                ...
                                                sx={{ color: '#E2E8F0' }}
                                            >
                                                <CheckCircle sx={{ mr: 1, fontSize: '1rem', color: '#10B981' }} />
                                                <strong>Data da Prescrição:</strong> {formatDate(prescription.prescriptionDate)}
                                            </Typography>
                                            {prescription.validUntil && (
                                                <Typography
                                                    ...
                                                    sx={{ color: '#E2E8F0' }}
                                                >
                                                    <Warning sx={{ mr: 1, fontSize: '1rem', color: isExpired(prescription.validUntil) ? '#EF4444' : '#10B981' }} />
                                                    <strong>Válida até:</strong> {formatDate(prescription.validUntil)}
                                                    <Chip
