...
export default function ObservationSuggestions({
  ...
  const generateSuggestions = async () => {
    if (!diagnosis.trim() || medicines.length === 0) {
      setError(
        'Diagnóstico e medicamentos são necessários para gerar sugestões'
      );
      return;
    }
    ...
    try {
      const response = await PrescriptionsService.suggestObservations(
        diagnosis,
        medicines
      );
    ...
    >
      ...
      <Box mb={2}>
        <Box display="flex" alignItems="center" gap={2} mb={1.5}>
          ...
          >
            <strong style={{ color: '#F1F5F9' }}>Diagnóstico:</strong>{' '}
            {diagnosis}
          </Typography>
        </Box>
        <Box display="flex" alignItems="center" gap={2} mb={1.5}>
      ...
      {suggestions.length > 0 && (
        <>
          ...
          <Typography
            variant="subtitle1"
            sx={{ fontWeight: 600, mb: 1.5, color: '#F1F5F9' }}
          >
                  ...
                  primaryTypographyProps={{
                    ...
                    sx: {
                      color: selectedSuggestions.includes(suggestion)
                        ? '#3B82F6'
                        : '#E2E8F0',
                      fontWeight: selectedSuggestions.includes(suggestion)
                        ? 600
                        : 400,
                    },
                  }}
