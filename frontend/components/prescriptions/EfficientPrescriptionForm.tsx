...
import { Cid, Medicine, Patient } from '@/lib/types';
...
export default function EfficientPrescriptionForm({
  onSuccess,
  onCancel,
}: EfficientPrescriptionFormProps) {
  const { cids } = useCids();
  ...
  const handleCidSelect = (cidId: number | null) => {
    ...
    if (cidId) {
      ...
      if (cid && cid.medicines && cid.medicines.length > 0) {
        ...
        const medicineTexts = cid.medicines
          .map(
            medicine =>
  ...
  const handleMedicineSelect = (medicine: Medicine | null) => {
    if (medicine) {
      ...
      if (!isAlreadyInPrescription) {
        ...
        showWarning('Este medicamento já foi adicionado à prescrição.');
      }
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await PrescriptionsService.createPrescription({
        ...
        diagnosis: selectedCidData
          ? `${selectedCidData.code} - ${selectedCidData.name}`
          : '',
        prescription,
        observations,
      ...
      showError(err.response?.data?.message || 'Erro ao criar prescrição');
    } finally {
      setSaving(false);
    }
  };
...
${observations}
    ...
    try {
      ...
      showSuccess('Prescrição copiada para a área de transferência!');
    } catch (error) {
      console.error('Erro ao copiar:', error);
      showError('Erro ao copiar prescrição para área de transferência');
    }
  };
        ...
        <Box component="form" onSubmit={handleSubmit}>
          <Grid container spacing={3}>
            ...
            <Grid item xs={12}>
              <Typography
                variant="body2"
                sx={{ mb: 2, color: '#94A3B8', fontStyle: 'italic' }}
              >
                💡 Os medicamentos do CID selecionado foram preenchidos
                automaticamente. Use o campo abaixo para adicionar medicamentos
                extras se necessário.
              </Typography>
              <Autocomplete
                options={medicines}
                getOptionLabel={option =>
                  `${option.name} ${option.dosage} - ${option.manufacturer}`
                }
            ...
            {selectedCidData && selectedMedicines.length > 0 && (
              <Grid item xs={12}>
                ...
                >
                  {showSuggestions ? 'Ocultar' : 'Mostrar'} Sugestões de
                  Observações (IA) - CID: {selectedCidData?.code}
                </Button>
              </Grid>
            )}
