...
import { useDetailsModal, useListState, useMedicines } from '@/hooks';
import { Medicine } from '@/types/entities.types';
...
// Interface Medicine agora importada de @/types/entities.types
...
export default function MedicinesList() {
  ...
  const { medicines, isLoading: loading, mutate: refetch } = useMedicines();
  const {
    selectedId,
    isOpen: showDetails,
    openDetails,
  ...
  const openDetailsView = (id: number) => {
    setEditMode(false);
    openDetails(id);
  };

  const openDetailsEdit = (medicine: Medicine) => {
    setEditMode(true);
    openDetails(medicine.id);
  };

  const openDetailsCreate = () => {
    setEditMode(true);
    openDetails(0); // ID especial para criar novo
  };
  ...
  const filteredAndSortedMedicines = useMemo(() => {
    ...
    const searchFields: (keyof Medicine)[] = [
      'name',
      'genericName',
      'manufacturer',
    ];
    ...
    const sortConditions = {
      ...
      manufacturer: (a: Medicine, b: Medicine) =>
        a.manufacturer.localeCompare(b.manufacturer),
      createdAt: (a: Medicine, b: Medicine) =>
        new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime(),
      isActive: (a: Medicine, b: Medicine) =>
                      ...
                      <Box display="flex" gap={1}>
                        ...
                        >
                          <IconButton
                            ...
                            sx={{
                              color: favorites.has(medicine.id)
                                ? '#F59E0B'
                                : '#64748B',
                              '&:hover': {
                                color: favorites.has(medicine.id)
                      ...
                      <Box mb={2}>
                        ...
                        {medicine.sideEffects && (
                          ...
                          >
                            ...
                            >
                              Efeitos colaterais:{' '}
                              {medicine.sideEffects.substring(0, 120)}...
                            </Typography>
                          </Alert>
                        )}
                      </Box>
                    ...
                    >
                      ...
                      <Box display="flex" gap={1}>
                        <Tooltip title="Ver detalhes">
                          <IconButton
                            size="small"
                            onClick={() => openDetailsView(medicine.id)}
                            sx={{
                              color: '#3B82F6',
                              '&:hover': {
                        ...
                        <Tooltip title="Editar">
                          <IconButton
                            size="small"
                            onClick={() => openDetailsEdit(medicine)}
                            sx={{
                              color: '#10B981',
                              '&:hover': {
